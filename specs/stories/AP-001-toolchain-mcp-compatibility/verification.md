...
`12.4.1`. Candidate source is the complete AP-001 diff based on revision
`8a4dff88aeb7b19bdee179ca257af0dec1655399`; the immutable implementation
revision and ForgePilot evidence are recorded after commit. No bearer value is
...
| AC-09 | blocked | Linux prerequisite table in `docs/toolchain-compatibility.md`                                                 | Designated target metadata covers OS, cgroup v2, service account, launcher, protected data path and credential source without secret values              | No designated Linux target, account, launcher, protected path or credential-source metadata was supplied. The current host is macOS. This blocks G0 acceptance and requires a ForgePilot Gate before handoff.                                                                                                                                                                                                             |
| AC-10 | blocked | This evidence table and `docs/toolchain-compatibility.md`                                                     | Every AC has allowed status, source revision, command/fixture, expected and actual result, date and limits; ForgePilot binds an immutable revision       | The complete evidence shape is present, but an immutable implementation revision and ForgePilot verification record do not exist until the reviewed candidate is committed.                                                                                                                                                                                                                                               |
...
AC-09 remains blocked, so G0 cannot be accepted. Claude positive query,
