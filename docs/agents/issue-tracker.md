...
ForgeFlow Story（`specs/stories/<ID>-<slug>/`），其執行狀態由 ForgePilot Work Item 管理。
...
若內容已是正式 implementation requirement，建立或更新 ForgeFlow Story，並以 ForgePilot Work Item 管理執行；不要另建 `.scratch` implementation 狀態。
